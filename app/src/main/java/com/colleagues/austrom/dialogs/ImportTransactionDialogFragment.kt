...
class ImportTransactionDialogFragment(private var transaction: Transaction, private var transactions: List<Transaction>,  private var activity: AppCompatActivity) : DialogFragment() {
    fun setOnDialogResultListener(l: ((List<Transaction>)->Unit)) { returnResult = l }
    private var returnResult: (List<Transaction>)->Unit = {}
    //region Binding
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        acceptButton.setOnClickListener { updateTransactionsWithNewValues(); dismiss()}
        cancelButton.setOnClickListener { dismiss() }
        return targetSelectionDialog
    }

    private fun updateTransactionsWithNewValues() {
        val updatedTransactionsList = mutableListOf<Transaction>()
        ...
        transactions.forEach { _transaction ->
            if (_transaction.categoryId == unknownCategory.text.toString() && _transaction.transactionType()==transaction.transactionType()) {
                ...
                updatedTransactionsList.add(_transaction)
            }
        }
        returnResult(updatedTransactionsList)
    }
